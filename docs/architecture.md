...
The `/my-ballot/` route provides a no-server-storage election lookup. A visitor-entered address is sent directly from the browser to the U.S. Census Geocoder using its documented JSONP interface; the address is never sent to or stored by Smarter.Vote. The returned state and congressional district plus matched race IDs are retained in tab-scoped `sessionStorage` so results survive a refresh, then cleared when the visitor searches another address or closes the tab. Results transition in place on the focused route rather than returning to the homepage. The lookup is not a complete ballot and does not identify other state, local, judicial, or ballot-measure contests. There is no autocomplete provider, Smarter.Vote address endpoint, or durable address persistence. The launch also has no Stripe Checkout, subscriptions, donation processing, or partner payment portal. Correction reports use public GitHub issues; other public inquiries currently use mail links rather than a server-side CRM or form-processing service.
