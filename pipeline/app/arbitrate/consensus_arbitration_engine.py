...
from ..schema import Summary, ConfidenceLevel, ArbitrationResult, LLMResponse, CanonicalIssue
...
class ConsensusArbitrationEngine:
    ...
    async def arbitrate_summaries(self, summaries: List[Summary], 
                                context: Dict[str, Any] = None) -> ArbitrationResult:
        """
    ...
    def _find_consensus_groups(self, summaries: List[Summary], 
                             similarity_matrix: List[List[float]]) -> List[List[int]]:
        """
    ...
    async def _create_consensus_result(self, consensus_indices: List[int], 
                                     summaries: List[Summary]) -> ArbitrationResult:
        """
        ...
        weighted_confidence = sum(
            self.model_weights.get(s.model, 1.0) * self._confidence_to_float(s.confidence)
    ...
    async def _handle_disagreement(self, consensus_groups: List[List[int]], 
                                 summaries: List[Summary]) -> ArbitrationResult:
        """
