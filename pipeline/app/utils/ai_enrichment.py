...
def ai_enrich(content_text: str, metadata: Optional[Dict[str, Any]] = None) -> AIAnnotations:
    ...
    try:
        ...
        return AIAnnotations(
            usefulness={"score": 0.5, "reasons": ["enrichment_failed"]}
        )
        ...
        for issue in issues:
            ...
            if candidate.lower() in text.lower() and any(keyword in text.lower() for keyword in _get_issue_keywords(issue)):
                ...
                for sentence in sentences:
                    if candidate.lower() in sentence.lower() and any(keyword in sentence.lower() for keyword in _get_issue_keywords(issue)):
                        evidence.append({
...
def _extract_claims(text: str) -> List[Dict[str, Any]]:
    ...
    for i, sentence in enumerate(sentences):
        ...
        if len(sentence.split()) > 6 and any(keyword in sentence.lower() for keyword in [
            "will", "should", "must", "believes", "supports", "opposes", "plans", "proposes"
        ]):
    ...
    for sentence in sentences[:3]:  # Limit to first few sentences
        if len(sentence.split()) > 8:
            ...
            if "support" in sentence.lower() or "oppose" in sentence.lower():
                question = f"What is the position on {sentence.split()[-3:][0] if len(sentence.split()) > 3 else 'this issue'}?"
                qa_pairs.append({
                    "q": question,
                    "a": sentence,
