...
try:
    from ..schema import ExtractedContent, Source, CanonicalIssue
except ImportError:
    # Fallback for direct imports
    from shared.models import ExtractedContent, Source, CanonicalIssue
...
class ContentExtractor:
    ...
    def __init__(self):
        ...
        self._issue_keywords = {
            CanonicalIssue.HEALTHCARE: ["healthcare", "health care", "medicare", "medicaid", "insurance", "hospital", "medical"],
            ...
            CanonicalIssue.REPRODUCTIVE_RIGHTS: ["abortion", "reproductive", "roe v wade", "planned parenthood", "birth control"],
    ...
    async def _extract_single_item(self, item: Dict[str, Any]) -> Optional[ExtractedContent]:
        ...
        usefulness_score, is_useful, usefulness_reasons = self._calculate_usefulness(
            text, word_count, language, language_confidence, entity_hits, issue_hits, 
            structured_metadata, tables, item
        )
        ...
        enriched_metadata = {
            ...
            **item.get("headers", {}),
            ...
            "original_size_bytes": item.get("content_length", len(content.encode() if isinstance(content, str) else content or b'')),
        ...
        def extract_text_recursive(obj, depth=0, max_depth=6):
            ...
            if isinstance(obj, dict):
                ...
                priority_fields = [
                    ...
                    "title", "name", "headline", "message", "comment"
                ]
    ...
    def _calculate_usefulness(self, text: str, word_count: int, language: str, language_confidence: float,
                             entity_hits: Dict[str, List[str]], issue_hits: Dict[str, int], 
                             structured_metadata: Dict[str, Any], tables: List[Dict[str, Any]], 
                             original_item: Dict[str, Any]) -> Tuple[float, bool, List[str]]:
        """
