...
<div class="container mx-auto px-4 py-6 sm:py-8 max-w-7xl">
  ...
  <header class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
      <nav class="mb-1 text-xs text-content-subtle uppercase tracking-wide">
        ...
        <a href="/races/{slug}{isDraftPreview ? '?draft=true' : ''}" class="hover:text-content">Race Detail</a> &middot;
        <span class="text-content-muted">Compare</span>
      </nav>
      <h1 class="text-2xl sm:text-3xl font-extrabold text-content tracking-tight">Compare Candidates</h1>
      {#if race}
  ...
  {#if loading}
    ...
    <div class="space-y-6">
      <div class="h-16 bg-surface border border-stroke rounded-xl animate-pulse" />
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="h-96 bg-surface border border-stroke rounded-xl animate-pulse md:col-span-3" />
      </div>
    </div>
  {:else if error}
    <div class="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-lg p-6 text-center">
      <h2 class="text-xl font-bold text-red-800 dark:text-red-200 mb-2">Error</h2>
      <p class="text-red-600 dark:text-red-400">{error}</p>
      <a href="/races/{slug}" class="mt-4 inline-block text-blue-600 dark:text-blue-400 font-semibold underline">
        Return to Race Overview
      </a>
    </div>
  {:else if race && candidates.length === 0}
    <div class="bg-surface border border-stroke rounded-xl p-8 text-center text-content-subtle">
      <p class="text-lg font-semibold text-content">No Candidates Selected</p>
      <p class="mt-2 text-sm">Please select candidates from the race detail page to compare them.</p>
      <a href="/races/{slug}" class="mt-4 inline-flex btn-primary no-underline text-sm font-semibold">
      ...
      <div class="min-w-[768px]">
            ...
            {#each candidates as candidate}
              <div class="px-6 flex items-center gap-3 border-r border-stroke last:border-none">
                {#if candidate.image_url}
                  ...
                  />
                {:else}
                  <div class="w-12 h-12 rounded-full bg-blue-100 dark:bg-blue-900/40 flex items-center justify-center flex-shrink-0 text-sm font-bold text-blue-700 dark:text-blue-300">
                    {candidate.name.split(' ').map(n=>n[0]).slice(0,2).join('')}
                  </div>
                {/if}
                <div class="min-w-0">
                  <a href="/races/{race.id}/{candidateSlug(candidate.name)}{isDraftPreview ? '?draft=true' : ''}" class="font-extrabold text-sm text-content hover:text-blue-600 block truncate">
                    {candidate.name}
                  </a>
                  <div class="flex items-center gap-1.5 mt-0.5">
                    {#if candidate.party}
                      <span class="inline-block px-1.5 py-0.5 rounded text-[10px] font-bold bg-surface-alt border border-stroke text-content-muted capitalize leading-none">
                        {partyAbbr(candidate.party)}
                      </span>
                    {/if}
                    {#if candidate.incumbent}
                      <span class="inline-block px-1.5 py-0.5 rounded text-[10px] font-bold bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 text-green-700 dark:text-green-300 leading-none">
                        Incumbent
                      </span>
                    {/if}
                  </div>
        ...
        <div class="divide-y divide-stroke">
          ...
          <div class="grid" style="grid-template-columns: 220px repeat({candidates.length}, 1fr)">
            ...
            {#each candidates as candidate}
              <div class="p-6 text-sm text-content-muted leading-relaxed border-r border-stroke last:border-none">
                {candidate.summary}
              ...
              {#each candidates as candidate}
                ...
                <div class="p-6 border-r border-stroke last:border-none flex flex-col gap-3">
                  {#if stance}
                    ...
                    {/if}
                  {:else}
                    <span class="text-xs text-content-faint italic select-none">No stance researched yet.</span>
                  {/if}
                </div>
              {/each}
            </div>
          ...
          <div class="grid" style="grid-template-columns: 220px repeat({candidates.length}, 1fr)">
            ...
            {#each candidates as candidate}
              <div class="p-6 border-r border-stroke last:border-none text-sm text-content-muted">
                    ...
                    {#each candidate.career_history as entry}
                      <div class="border-l-2 border-blue-500/50 pl-3 py-0.5">
                        <div class="flex items-baseline justify-between gap-2 flex-wrap">
                          <span class="font-semibold text-content text-xs">{entry.title}</span>
                          {#if entry.start_year}
                        ...
                        {#if entry.organization}
                          <span class="text-xs text-content-subtle block">{entry.organization}</span>
                        {/if}
                      </div>
                    {/each}
                  </div>
                {:else}
                  <span class="text-xs text-content-faint italic select-none">No career records.</span>
                {/if}
              </div>
            {/each}
          </div>
          ...
          <div class="grid" style="grid-template-columns: 220px repeat({candidates.length}, 1fr)">
            ...
            {#each candidates as candidate}
              <div class="p-6 border-r border-stroke last:border-none text-sm text-content-muted">
                {#if candidate.education && candidate.education.length > 0}
                  <div class="space-y-3">
                    {#each candidate.education as edu}
                      <div>
                        <span class="font-semibold text-content text-xs block">{edu.institution}</span>
                ...
                {:else}
                  <span class="text-xs text-content-faint italic select-none">No education records.</span>
                {/if}
              </div>
            {/each}
          </div>
            ...
            {#each candidates as candidate}
              <div class="p-6 border-r border-stroke last:border-none text-sm text-content-muted">
                {#if candidate.donor_summary}
                  ...
                  {/if}
                {:else}
                  <span class="text-xs text-content-faint italic select-none">No donor records available.</span>
                {/if}
              </div>
            {/each}
          </div>
            ...
            {#each candidates as candidate}
              <div class="p-6 border-r border-stroke last:border-none text-sm text-content-muted">
                {#if candidate.voting_summary}
                  ...
                  {/if}
                {:else}
                  <span class="text-xs text-content-faint italic select-none">No voting summary available.</span>
                {/if}
              </div>
            {/each}
          </div>
