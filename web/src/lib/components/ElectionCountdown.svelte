...
<div
  class="bg-gradient-to-r from-blue-500/10 to-red-500/10 border border-stroke rounded-2xl p-4 sm:p-5 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4"
>
  <div class="space-y-1">
  ...
  {#if status === "upcoming"}
    <div class="flex gap-2 sm:gap-3 justify-center">
      <!-- Days -->
      <div
        class="flex flex-col items-center bg-surface border border-stroke rounded-xl px-2.5 py-1.5 min-w-[56px] shadow-sm"
      >
        <span class="text-lg font-black text-content tabular-nums leading-none"
          >{days}</span
        >
        <span
          class="text-[9px] font-bold text-content-subtle uppercase tracking-wider mt-1"
          >Days</span
        >
      </div>
      <!-- Hours -->
  ...
  {:else}
    <div
      class="px-4 py-2 rounded-xl text-sm font-black border uppercase tracking-wider shadow-sm
      {status === 'today'
