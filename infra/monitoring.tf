...
# ---------------------------------------------------------------------------
# Cloud Monitoring — admin-agent Cloud Function (gen2)
# Gen2 functions run on Cloud Run under the hood but still report the classic
# Cloud Functions execution metrics under resource.type = "cloud_function"
# (function_name / region labels); that's what these two alerts use, gated on
# the same alert_email conditional plus the function actually being enabled.
# ---------------------------------------------------------------------------

# Alert: admin-agent non-ok execution rate > 0.02/sec sustained for 5 minutes
resource "google_monitoring_alert_policy" "admin_agent_errors" {
  count        = var.alert_email != "" && var.enable_admin_agent_function ? 1 : 0
  project      = var.project_id
  display_name = "admin-agent High Error Rate (${var.environment})"
  combiner     = "OR"

  conditions {
    display_name = "Non-ok executions > 0.02/sec for 5 minutes"

    condition_threshold {
      filter          = "resource.type = \"cloud_function\" AND resource.labels.function_name = \"admin-agent-${var.environment}\" AND metric.type = \"cloudfunctions.googleapis.com/function/execution_count\" AND metric.labels.status != \"ok\""
      duration        = "300s"
      comparison      = "COMPARISON_GT"
...
# Alert: admin-agent hard execution failures (crash or timeout, i.e. the
# runtime never returned a normal "ok"/"error" status at all) — any occurrence
# in a 15-minute window. These are infra-level failures rather than the
# agent's own reported error statuses, so even a single one is worth paging on.
resource "google_monitoring_alert_policy" "admin_agent_execution_failures" {
  count        = var.alert_email != "" && var.enable_admin_agent_function ? 1 : 0
  project      = var.project_id
  display_name = "admin-agent Execution Failures (${var.environment})"
  combiner     = "OR"

  conditions {
    display_name = "Crash or timeout executions > 0 in 15 minutes"

    condition_threshold {
      filter          = "resource.type = \"cloud_function\" AND resource.labels.function_name = \"admin-agent-${var.environment}\" AND metric.type = \"cloudfunctions.googleapis.com/function/execution_count\" AND (metric.labels.status = \"crash\" OR metric.labels.status = \"timeout\")"
      duration        = "0s"
      comparison      = "COMPARISON_GT"
      threshold_value = 0
